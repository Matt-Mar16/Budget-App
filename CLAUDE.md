...
- Run the tests (from `ledger/`): `.venv/Scripts/python.exe -m pytest` (161 tests across `tests/test_finance_core.py`, `tests/test_budget_app.py`, `tests/test_profiles.py`, `tests/test_inbox_sync.py`)
...
`enable_drag_reorder(handles_and_rows, on_reorder)` (`budget_app.py`, module-level, right after `Card`) is a small reusable helper for any vertically-`pack()`'d list of rows that wants drag-to-reorder: takes `(handle_widget, row_widget, item_id)` triples, live-swaps a row past whichever sibling the pointer crosses (comparing against each sibling's vertical midpoint) by repacking in the new order, then calls `on_reorder(ids_in_final_order)` once the drag ends. Dragging is bound to a small grip glyph (`⠿`) rather than the whole row, so the row's own interactive widgets (entries, buttons) keep working normally. Currently wired into `BudgetsTab` (need/want/saving/income category cards, replacing the old Up/Down-button mechanism); no other list in the app is manually orderable yet, but any future one can call this directly instead of re-implementing the drag mechanics.
